...
namespace SMPScripts
{
    ...
    {
        public float batteryCapacity = 1000f; // Capacitatea totală a bateriei în Jouli
        public float currentEnergy = 500f; // Energia curentă stocată în baterie
    }
    public class MotoController : MonoBehaviour
    {
        ...
        [Header("Regenerative Braking Settings")]
        public float regenEfficiency = 0.8f; // Eficiența recuperării energiei

        [Header("Battery Settings")]
        public BatterySettings batterySettings;

        [Header("UI References")]
        public UIController uiController; // Referință la scriptul UI

        private float energyRecovered = 0f; // Energia totală recuperată
        private float energyConsumed = 0f; // Energia totală consumată
        ...
        void FixedUpdate()
        {
            ...
            //{
                if (rawCustomAccelerationAxis > 0)
                {
                    float energyConsumption = engineSettings.torque * rawCustomAccelerationAxis * Time.fixedDeltaTime;
                    batterySettings.currentEnergy -= energyConsumption;
                    energyConsumed += energyConsumption;
                    if (batterySettings.currentEnergy < 0f)
                        batterySettings.currentEnergy = 0f;
                }
            //}

            if (uiController != null)
            {
                uiController.UpdateUI(rb.velocity.magnitude, energyRecovered, energyConsumed, batterySettings.currentEnergy, batterySettings.batteryCapacity);
            }
        }
        void Update()
        {
        ...
        void ApplyRegenerativeBrakes()
        {
            Vector3 brakeForce = -rb.velocity.normalized * engineSettings.torque;
            rb.AddForce(brakeForce);

            float currentSpeed = rb.velocity.magnitude;
            float recoveredEnergy = brakeForce.magnitude * currentSpeed * Time.fixedDeltaTime * regenEfficiency;
            energyRecovered += recoveredEnergy;

            batterySettings.currentEnergy += recoveredEnergy;
            if (batterySettings.currentEnergy > batterySettings.batteryCapacity)
            {
                batterySettings.currentEnergy = batterySettings.batteryCapacity;
            }

            float speedReductionFactor = 0.1f;
            rb.velocity = rb.velocity * (1 - speedReductionFactor * regenEfficiency * Time.fixedDeltaTime);
        }
